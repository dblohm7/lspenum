...
{
  int result = WideCharToMultiByte(CP_UTF8, 0, aStr, aLenExcl, nullptr, 0, nullptr, nullptr);
  if (!result) {
    return string();
  }
  auto str = make_unique<char[]>(result);
  result = WideCharToMultiByte(CP_UTF8, 0, aStr, aLenExcl, str.get(), result, nullptr, nullptr);
  if (!result) {
    return string();
  }
...
string ToUTF8(const wchar_t* aStr)
{
  return ToUTF8(aStr, wcslen(aStr));
}
...
extern "C" int wmain(int argc, wchar_t* argv[])
{
  ...
  for (int i = 0; i < result; ++i) {
    ...
    int status = WSCGetProviderPath(&p.ProviderId, provPath, &provPathLen, &errno);
    if (!status) {
    ...
    o << entryIndent << "Provider Flags: 0x" << hex << p.dwProviderFlags << endl;
    DWORD categoryInfo = 0;
    size_t categoryInfoSize = sizeof(categoryInfo);
    status = WSCGetProviderInfo(&p.ProviderId, ProviderInfoLspCategories, (PBYTE)&categoryInfo, &categoryInfoSize, 0, &errno);
    if (!status) {
